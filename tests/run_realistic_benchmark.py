...
SITE_ROOT = ROOT / "tests" / "realistic_site"
...
async def _run_benchmark(max_seconds):
    ...
    try:
        print("===================================")
        print("   ARGUS REALISTIC SITE BENCHMARK")
        print("===================================")
...
def main():
    parser = argparse.ArgumentParser(description="ARGUS 현실형 공공 게시판 end-to-end benchmark")
    parser.add_argument("--max-seconds", type=float, default=180.0)
    parser.add_argument(
        "--strict",
        action="store_true",
        help="페이지 범위/precision/recall/detector recall 중 하나라도 기준 미달이면 exit code 1",
    )
